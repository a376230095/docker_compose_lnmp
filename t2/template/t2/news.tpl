...
if($id==0){
  ...
  if(strpos($row["N_content"],"[fh_free]")!==false || p($row["N_price"])==0){
    $intro=mb_substr(strip_tags(splitx($row["N_content"],"[fh_free]",0)),0,200,"utf-8");
  }else{
    $intro="内容加密";
  }
        
          $api=$api."<li><a class=\"h4 bold\" href=\"?type=newsinfo&id=".$row["N_id"]."\" style=\"\">".$row["N_title"]."</a>
          <div class=\"m-b-md\">
           
           <div class=\"media-body\">
            <div class=\"m-l-sm\">".$intro."</div>
            <div class=\"pull-right font-12\" style=\"color: #aaa\">
             ".$row["N_author"]." [
             
             <span>".date("Y-m-d",strtotime($row["N_date"]))."</span> ] 
            </div>
           </div>
          </div></li>";
          ]]
...
<div class="m-t-md p-l-md b-l b-light hidden-xs hidden-sm">
 ...
 <div class="m-t-sm lh20" align="center">
  <div class="text-left" style="max-width: 300px;">
    ...
    <fh-function>
      $i=1;
 $sql="select * from sl_news,sl_nsort where S_id=N_sort and N_sh=1 and S_del=0 and N_del=0 ".$M_ninfo."  order by N_date desc limit 10";
          s[[$api=$api."<i>".$i."&nbsp;</i>
 ...
 <div class="m-t-sm">
  <div class="row text-center m-none">
     ...
     <fh-function>
        $sql="select * from sl_product where P_sh=1 and P_del=0 ".$M_pinfo."  order by P_top desc,P_id desc limit 4";
